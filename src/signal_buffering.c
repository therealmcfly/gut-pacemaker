...
int signal_buffering(double *in_signal, size_t signal_length, int *channel_num, char *file_name)
{
	...
	while (j < signal_length) // Keep processing until reaching the signal length
	{
		...
		if (lowpass_filter(buffer, lowpass_signal, cur_buffer_size))
		{
		...
		/* --------------------------- HIGH PASS FILTERING -----------------------------*/
		/* -----------------------------------------------------------------------------*/
		if (highpass_filter(lowpass_signal, cur_buffer_size, hpf_signal, &hpf_signal_len))
		{
		...
		if (check_processing_result(hpf_signal, cur_buffer_size + HPF_FILTER_ORDER, *channel_num, file_name, "hpf", shift))
		{
		...
		// 		if (remove_artifacts(hpf_signal, artifact_signal, cur_buffer_size + HPF_FILTER_ORDER))
		// 		{
		...
		// 		if (check_processing_result(artifact_signal, cur_buffer_size, *channel_num, file_name, "ad", shift))
		// 		{
